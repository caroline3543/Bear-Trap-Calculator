...
function ScreenshotScanner({ lang, accounts, activeAccountId, onApply }) {
  ...
  async function runScan() {
    ...
    try {
      ...
      setConfidence(best.parsed.confidence);
  ...
  const looksOff = hasScanned && missingCount >= 2 && missingCount < SCAN_FIELDS.length;
  // Every candidate language (or the single one tried, with auto-detect
  // off) came back with nothing for all 6 fields — meaningfully different
  // from "found some, not others," and "try a different language" is
  // actively misleading here when auto-detect already tried several.
  const totalFailure = hasScanned && missingCount === SCAN_FIELDS.length;
      ...
      {!imgSrc ? (
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            ...
            <Btn tone="ghost" small onClick={() => { setImgSrc(null); setEdited(null); setScannedValues(null); setHasScanned(false); setFileError(null); }}>✕</Btn>
          </div>
